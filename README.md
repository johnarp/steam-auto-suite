<div align="center">

![Banner](./assets/banner_suite.png)

# Steam Auto Suite

A collection of tools for managing your Steam library.

[![License](https://img.shields.io/github/license/johnarp/steam-auto-suite?style=for-the-badge)](./LICENSE)
![Version](https://img.shields.io/github/v/release/johnarp/steam-auto-suite?style=for-the-badge)

![Python](https://img.shields.io/badge/Python-3772a2?style=for-the-badge&logo=python&logoColor=white)

</div>

## 🧰 Tools

| Tool | Description |
| - | - |
| [Steam Auto Art](./tools/steam-auto-art/) | Automatically applies your custom Steam artwork to your library. |
| [Steam Auto Collections](./tools/steam-auto-collections/) | Automatically creates and populates your Steam collections. |
| [Steam Auto Favorites](./tools/steam-auto-favorites/) | Automatically populates your Steam favorites. |
| [Steam Auto ID](./tools/steam-auto-id/) | Find a Steam game's App ID by name. |
| [Steam Auto Launch](./tools/steam-auto-launch/) | Automatically applies launch options to your Steam games. |
| [Steam Auto Name](./tools/steam-auto-name/) | Find a Steam game's name by App ID. |
| [Steam Auto Playtime](./tools/steam-auto-playtime/) | Automatically sets playtime for your Steam games. |

## 🚀 Installation

### 1. Download the Code

```
git clone https://github.com/johnarp/steam-auto-suite
cd steam-auto-suite
```

### 2. Follow the Steps

Each tool is a separate folder, containing the script and their individual instructions in a README.md file.

To use the tool, follow the instructions.

## 🗺️ Roadmap

### New Tools

- **Steam Auto Add:** Automatically add non-Steam games
- **Steam Auto Clear:** Automatically clear things like custom art, logo position, and launch options
- **Steam Auto Controller:** Quickly apply controller settings
- **Steam Auto Install:** Provide a queue and automatically install those games
- **Steam Auto Uninstall:** The opposite of install
- **Steam Auto Visibility:** Automatically hide or make visible

### Existing Tools

- **Steam Auto Art:** Argparsing to allow more than the top result for a single item or the entire queue
- **Steam Auto Art:** Icon functionality

## 📜 Disclaimer

Several tools in this suite may read from and write directly to files on your computer (e.g. configuration files or Steam-related files) in order to function. This happens only when you run the tool, it does not run in the background or schedule anything automatically. Please review each tool's documentation before use and ensure you understand what files may be modified.

"Auto" refers to performing repetitive, mundane, and/or lengthy tasks.

## 📄 License

This project is licensed under the [MIT License](./LICENSE).

## ⚖️ Legal

Steam Auto Suite is an unofficial project and is not affiliated with, endorsed by, or associated with Valve Corporation or Steam in any way. Some tools use Steam's publicly available endpoints. 

"Steam" is a trademark of Valve Corporation.