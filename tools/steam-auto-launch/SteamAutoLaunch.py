...
# The folder the script is in, is the same as your defined launch options.
# Your launch options file must be named "launch_options.json".
LAUNCH_OPTIONS_FILE = os.path.join(os.getcwd(), "launch_options.json")


def set_launch_options(LINES, APP_ID, OPTIONS):
    ...
    while i < len(LINES):
        ...
        if LINES[i].strip() == f'"{APP_ID}"':
            ...
            if j < len(LINES) and LINES[j].strip() == '{':
                ...
                # Search within the block for an existing LaunchOptions entry.
                INNER_INDENT = INDENT + '\t'
                FOUND = False
                for m in range(j + 1, CLOSE):
                    if LINES[m].strip().startswith('"LaunchOptions"'):
                        # Replace it with the new value.
                        LINES[m] = f'{INNER_INDENT}"LaunchOptions"\t\t"{OPTIONS}"\n'
                        FOUND = True
                        break

                # If there was no existing LaunchOptions, insert one before the closing brace.
                if not FOUND:
                    LINES.insert(CLOSE, f'{INNER_INDENT}"LaunchOptions"\t\t"{OPTIONS}"\n')
...
def main():
    ...
    # Gives an error if launch_options.json doesn't exist.
    if not os.path.exists(LAUNCH_OPTIONS_FILE):
        print(f"launch_options.json not found: {LAUNCH_OPTIONS_FILE}")
        return
    ...
    # Loads your launch_options.json into Python.
    with open(LAUNCH_OPTIONS_FILE, "r") as f:
    ...
    # For each app ID and its launch options, try to apply them.
    for APP_ID, OPTIONS in LAUNCH_OPTIONS.items():
        SUCCESS = set_launch_options(LINES, str(APP_ID), OPTIONS)
        if SUCCESS:
            print(f"[OK]: {APP_ID} --> {OPTIONS}")
        else:
